...
void bubbleSort(vector<int>& arr) {
    int n = arr.size();
    for (int i = 0; i < n - 1; ++i) {
        int cambio = 0; //Funcion para detectar el cambio
        ...
        if(cambio == 1){
            for(int k = 0; k < arr.size(); k++){ //Imprimir conforme el ordenamiento siempre que se realice un cambio
                cout << arr[k] << " ";
            }
            cout << endl;
        }
...
void menuOrdenamiento() {
    int lon;
    cout << "Ingrese la longitud del vector: "; cin >> lon;
    vector<int> arr(lon);
    for(int i = 0; i < lon; i++){
        arr[i] = rand() % 100 + 1;
    }
...
int fibonacciRecursivo(int n) {
    ...
    int val = fibonacciRecursivo(n - 1) + fibonacciRecursivo(n - 2);
    return val;
}
...
// Menú para Árbol Binario de Búsqueda
void menuArbol() {
    BST arbol;
    int opc, val;
